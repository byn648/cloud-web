...
function generatePodTemplateMetadata(
  ...
  const schedulingAnnotations = schedulingStore.getPodTemplateAnnotations()
...
function generatePodSpec(): V1PodSpec {
  ...
  const spec: V1PodSpec = {
    ...
    ...((schedulingSpec.schedulerName || advancedSpec.schedulerName) && {
      schedulerName: schedulingSpec.schedulerName || advancedSpec.schedulerName
    }),
...
export function generatePodFromStores(): V1Pod {
  ...
  const baseMeta = generateMetadata(namespace, resourceName, nameEn, version, nameCn, desc, labels, annotations)
...
export function updateStoresFromDeployment(deployment: V1Deployment) {
  ...
  if (podSpec) {
    ...
    schedulingStore.loadFromK8s(podSpec, deployment.spec?.template?.metadata?.annotations)
...
export function updateStoresFromStatefulSet(statefulSet: V1StatefulSet) {
  ...
  if (podSpec) {
    ...
    schedulingStore.loadFromK8s(podSpec, statefulSet.spec?.template?.metadata?.annotations)
    advancedStore.loadFromK8s(podSpec)
  }
...
export function updateStoresFromDaemonSet(daemonSet: V1DaemonSet) {
  ...
  if (podSpec) {
    ...
    schedulingStore.loadFromK8s(podSpec, daemonSet.spec?.template?.metadata?.annotations)
    advancedStore.loadFromK8s(podSpec)
  }
...
export function updateStoresFromJob(job: V1Job) {
  ...
  if (podSpec) {
    ...
    schedulingStore.loadFromK8s(podSpec, job.spec?.template?.metadata?.annotations)
    advancedStore.loadFromK8s(podSpec)
  }
