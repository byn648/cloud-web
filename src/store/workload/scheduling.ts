...
export const GREEN_RL_SCHEDULER_NAME = 'green-rl-scheduler'
export const GREEN_SCHEDULE_PLAN_ID_ANNOTATION_KEY = 'green-scheduler.kube-nova.io/plan-id'
export const GREEN_SCHEDULE_TARGET_CLUSTER_ANNOTATION_KEY =
  'green-scheduler.kube-nova.io/target-cluster'
export const GREEN_SCHEDULE_PLACEMENT_ANNOTATION_KEY = 'green-scheduler.kube-nova.io/placement'
...
export const useSchedulingStore = defineStore('workload-scheduling', () => {
  ...
  const rlSchedulePlan = ref<RlSchedulePlan | null>(null)
  ...
  function setNodeSchedulingMode(mode: typeof nodeSchedulingMode.value) {
    ...
    if (mode !== 'auto') {
      rlSchedulePlan.value = null
    }
  }

  function setRlSchedulePlan(plan: RlSchedulePlan | null) {
    rlSchedulePlan.value = plan
  }

  function clearRlSchedulePlan() {
    rlSchedulePlan.value = null
  }

  /** 供 YAML / 确认页：与 store 中用户配置的 nodeAffinity 一致（无额外合并） */
  function getMergedNodeAffinity(): V1NodeAffinity | undefined {
    return affinity.value?.nodeAffinity
  }

  const effectiveNodeAffinity = computed(() => getMergedNodeAffinity())
  ...
  function validate() {
    ...
    if (nodeSchedulingMode.value === 'auto') {
      if (!rlSchedulePlan.value) {
        errors.push('请先生成分层强化学习调度计划')
      } else if (!rlSchedulePlan.value.executable) {
        errors.push(`当前分层强化学习调度计划不可执行：${rlSchedulePlan.value.reason}`)
      }
    }
  ...
  function getPodTemplateAnnotations(): Record<string, string> {
    if (nodeSchedulingMode.value !== 'auto') {
      return {}
    }
    const annotations: Record<string, string> = {}
    if (rlSchedulePlan.value) {
      annotations[GREEN_SCHEDULE_PLAN_ID_ANNOTATION_KEY] = rlSchedulePlan.value.planId
      if (rlSchedulePlan.value.targetCluster?.uuid) {
        annotations[GREEN_SCHEDULE_TARGET_CLUSTER_ANNOTATION_KEY] =
          rlSchedulePlan.value.targetCluster.uuid
      }
      annotations[GREEN_SCHEDULE_PLACEMENT_ANNOTATION_KEY] = JSON.stringify(
        rlSchedulePlan.value.placements.map((item) => ({
          replica: item.replicaName,
          node: item.nodeName
        }))
      )
    }
  ...
  function toK8sFormat() {
    ...
    } else if (nodeSchedulingMode.value === 'auto') {
      spec.schedulerName = GREEN_RL_SCHEDULER_NAME
    ...
    const mergedNode = getMergedNodeAffinity()
    if (mergedNode || affinity.value) {
      const cleanedAffinity: V1Affinity = {}
      if (mergedNode) {
        cleanedAffinity.nodeAffinity = mergedNode
      }
  ...
  // 从 K8s 格式加载
  function loadFromK8s(spec: any, podTemplateAnnotations?: Record<string, string>) {
    if (!spec) {
      reset()
      return
    }
