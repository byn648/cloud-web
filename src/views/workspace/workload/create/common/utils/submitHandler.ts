...
import { addDemoApplicationFromSubmit } from '@/views/workspace/application-demo/mock/demoStore'
import type { ResourceType as DemoResourceType } from '@/views/workspace/application-demo/mock/data'
...
export interface SubmitParams {
  ...
  schedulePlanJson?: string
  /** 应用中心演示：仅写本地 Mock，不请求后端 */
  demo?: boolean
}
...
/**
 * 演示模式：写入 application-demo 本地数据
 */
async function handleCreateAppDemo(params: SubmitParams): Promise<SubmitResult> {
  try {
    let replicaCount = 1
    if (params.resourceYamlStr) {
      try {
        const doc = yaml.load(params.resourceYamlStr) as { spec?: { replicas?: number } }
        if (typeof doc?.spec?.replicas === 'number') {
          replicaCount = doc.spec.replicas
        }
      } catch {
        /* ignore */
      }
    }
    const resourceTypeMap: Partial<Record<ResourceType, DemoResourceType>> = {
    ...
    const app = addDemoApplicationFromSubmit({
      workspaceId: params.workspaceId,
      nameCn: params.nameCn,
      nameEn: params.nameEn,
      description: params.description,
      resourceType: resourceTypeMap[params.resourceType] ?? 'deployment',
      replicaCount
    })
    return {
      success: true,
      message: '应用创建成功（演示数据，未请求后端）',
      applicationId: app.id
    }
  } catch (error: any) {
    return {
      success: false,
      message: error?.message || '演示创建失败',
      error
    }
  }
}
...
async function handleCreateApp(params: SubmitParams): Promise<SubmitResult> {
  if (params.demo) {
    return handleCreateAppDemo(params)
  }
  try {
    const submitData = {
      ...
      description: params.description || '',
      schedulePlanId: params.schedulePlanId,
      targetClusterUuid: params.targetClusterUuid,
      schedulePlanJson: params.schedulePlanJson
    }
