  ...
  <div class="resources-config">
    ...
    <ElAlert type="info" :closable="false" show-icon style="margin-bottom: 20px">
      ...
      <ul style="margin: 8px 0 0; padding-left: 20px; font-size: 13px; line-height: 22px">
        <li><strong>最小资源</strong>：容器启动所需的最小资源，用于调度决策</li>
        <li><strong>最大资源</strong>：容器可使用的最大资源，防止资源滥用</li>
        <li><strong>建议</strong>：最大资源设置为最小资源的 1.5-2 倍</li>
        <li
          ...
          >
            ...
            <div class="template-info">
              ...
              <div class="template-values">
                CPU 最小 {{ template.resources.cpuRequests }} / 最大 {{ template.resources.cpuLimits }}<br />
                内存 最小 {{ template.resources.memoryRequests }} / 最大 {{ template.resources.memoryLimits }}
              </div>
            </div>
          </div>
        </div>
...
<script setup lang="ts">
  ...
  withDefaults(
    defineProps<{
      /** 演示/简化创建流程：隐藏临时存储与快速配置模板 */
      demoSimplified?: boolean
    }>(),
    { demoSimplified: false }
  )
