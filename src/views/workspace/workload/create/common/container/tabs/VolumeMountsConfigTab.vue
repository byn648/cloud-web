<template>
  <div class="volume-mounts-config-tab">
    ...
    <div class="volume-mount-item volume-mount-item-example">
      <ElForm label-width="120px" size="small">
        <ElFormItem label="存储卷名称">
          <ElInput model-value="模型文件" readonly>
            <template #prepend>
              <HardDrive :size="14" />
            </template>
          </ElInput>
        ...
        <ElFormItem label="挂载路径">
          <ElInput model-value="/models" readonly>
            <template #prepend>
              <Folder :size="14" />
            </template>
          </ElInput>
        ...
        <ElFormItem label="说明">
          <span class="example-desc">face-model-pvc · 容器内读取人脸识别模型</span>
        </ElFormItem>
      </ElForm>
    </div>
    ...
    >
      <p style="margin: 0; font-size: 12px">
        请先在「存储配置」步骤添加「模型文件」存储卷，再点击「添加挂载」完成实际配置
      </p>
    </ElAlert>
...
<script setup lang="ts">
  ...
  import { Plus, Info, FolderOpen, Trash2, Folder, Zap, HardDrive } from 'lucide-vue-next'
