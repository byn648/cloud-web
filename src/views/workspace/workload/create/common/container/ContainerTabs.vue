<template>
  <div class="container-tabs">
    <ElTabs v-model="activeTab" type="border-card">
      ...
      <ElTabPane v-if="!demoSimplified" name="env">
        <template #label>
          <span class="tab-label">
            环境变量
      ...
      <ElTabPane name="resources">
        ...
        <ResourcesConfigTab :demo-simplified="demoSimplified" />
      </ElTabPane>
      ...
      <ElTabPane v-if="!demoSimplified" name="lifecycle">
        <template #label>
          <span class="tab-label">
            生命周期
            <ElTag
      ...
      <ElTabPane v-if="!demoSimplified" name="security">
        <template #label>
          <span class="tab-label">
            安全上下文
            <ElTag
      ...
      <ElTabPane v-if="!demoSimplified" name="advanced">
        <template #label>
          <span class="tab-label">
            高级配置
            <ElTag
...
<script setup lang="ts">
  ...
  const props = withDefaults(
    defineProps<{
      /** 演示/简化创建流程：隐藏环境变量、生命周期、安全上下文、高级配置 */
      demoSimplified?: boolean
    }>(),
    { demoSimplified: false }
  )

  const DEMO_HIDDEN_TABS = new Set(['env', 'lifecycle', 'security', 'advanced'])
  ...
  watch(
    () => props.demoSimplified,
    (simplified) => {
      if (simplified && DEMO_HIDDEN_TABS.has(activeTab.value)) {
        activeTab.value = 'basic'
      }
    },
    { immediate: true }
