    ...
    <div v-show="activeView === 'summary'" class="config-summary">
        ...
        <ElDescriptions :column="2" border size="default">
          ...
          <ElDescriptionsItem label="自定义标签" :span="2" label-class-name="desc-label">
            <template v-if="hasCustomLabels">
              <ElTag
                v-for="(value, key) in displayCustomLabels"
                :key="key"
                class="label-tag"
                type="info"
              >
      ...
      <ElCard class="summary-card">
        ...
        <ElDescriptions :column="2" border>
          <ElDescriptionsItem label="实例数量">
            <ElTag type="primary" size="large" effect="dark">
              {{ deploymentStore.replicas }} 个
            </ElTag>
          </ElDescriptionsItem>
          <ElDescriptionsItem v-if="!hideAdvancedSummary" label="更新策略">
            <ElTag :type="strategyType === 'RollingUpdate' ? 'success' : 'warning'">
              {{ strategyType }}
            </ElTag>
          </ElDescriptionsItem>
          <ElDescriptionsItem
            v-if="!hideAdvancedSummary && strategyType === 'RollingUpdate'"
            label="最大不可用"
          >
          ...
          <ElDescriptionsItem v-if="!hideAdvancedSummary" label="保留历史版本">
            {{ deploymentStore.revisionHistoryLimit }} 个
          </ElDescriptionsItem>
          <ElDescriptionsItem v-if="!hideAdvancedSummary" label="进度截止时间">
            {{ deploymentStore.progressDeadlineSeconds }} 秒
          </ElDescriptionsItem>
          <ElDescriptionsItem v-if="!hideAdvancedSummary" label="最小就绪秒数">
      ...
      <ElCard class="summary-card">
        ...
        <ElDescriptions :column="1" border size="default">
          <ElDescriptionsItem label="节点调度方式" label-class-name="desc-label">
            <template v-if="schedulingStore.nodeSchedulingMode === 'auto'">
              <ElTag type="success"> 自动调度 </ElTag>
            </template>
          ...
          <ElDescriptionsItem
            v-if="schedulingStore.nodeSchedulingMode === 'auto' && schedulingStore.rlSchedulePlan"
            label="RL 调度计划"
            label-class-name="desc-label"
          >
            <div class="rl-review">
              <div>
                <span class="review-muted">目标集群：</span>
                <ElTag type="success">
                  {{ schedulingStore.rlSchedulePlan.targetCluster?.name || '暂无可行集群' }}
                </ElTag>
              </div>
              <div>
                <span class="review-muted">模型版本：</span>
                {{ schedulingStore.rlSchedulePlan.modelVersion }}
              </div>
              <div class="placement-review">
                <ElTag
                  v-for="placement in schedulingStore.rlSchedulePlan.placements"
                  :key="placement.replicaName"
                  size="small"
                  type="info"
                >
                  {{ placement.replicaName }} → {{ placement.nodeName }}
                </ElTag>
              </div>
            </div>
          </ElDescriptionsItem>
          ...
          <ElDescriptionsItem
            :label="hideAdvancedSummary ? '节点过滤' : '节点亲和性'"
            label-class-name="desc-label"
            v-if="hasNodeAffinity"
          >
            <div v-if="schedulingStore.effectiveNodeAffinity">
              <ElTag
                v-if="
                  schedulingStore.effectiveNodeAffinity
      ...
      <!-- 高级配置 -->
      <ElCard v-if="!hideAdvancedSummary" class="summary-card">
        <template #header>
          <div class="card-header">
            <div class="header-left">
              <Settings :size="18" />
...
<script setup lang="ts">
  ...
  import { isDemoCreateRoute } from '../../../../application-demo/create/demoWorkloadBridge'
  ...
  const hideAdvancedSummary = computed(() => isDemoCreateRoute(route))
  ...
  // 计算属性
  const displayCustomLabels = computed(() => {
    const result: Record<string, string> = {}
    for (const [key, value] of Object.entries(metadataStore.metadata.labels)) {
      result[key] = value
    }
    return result
  })
  ...
  const hasCustomLabels = computed(() => {
    return Object.keys(displayCustomLabels.value).length > 0
  })
  ...
  const hasNodeAffinity = computed(() => {
    return !!schedulingStore.effectiveNodeAffinity
  })
  ...
  function validate() {
    ...
    if (schedulingStore.nodeSchedulingMode === 'auto' && !schedulingStore.rlSchedulePlan) {
      warnings.push('尚未生成分层强化学习调度计划')
    }
